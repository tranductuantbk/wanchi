...
with tab1:
    ...
    with st.form("form_them_sp"):
        ...
        with col1:
            ...
            # ĐÃ BỔ SUNG Ô CHI PHÍ KHÁC VÀO ĐÂY
            chi_phi_khac_chuan = st.number_input("Chi phí khác (Tem, bao bì...) (VNĐ)", min_value=0.0, step=1000.0)
        with col2:
            ...
            if not ma_sp.strip() or not ten_sp.strip():
                ...
                try:
                    c.execute("""INSERT INTO public.dm_san_pham 
                                 (ma_sp, ten_sp, gia_dai_ly, gia_khach_le, dinh_muc_nhua, don_gia_nhua, don_gia_cong, chi_phi_khac) 
                                 VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""", 
                              (ma_sp.strip(), ten_sp.strip(), gia_dai_ly, gia_khach_le_calc, dinh_muc_nhua, don_gia_nhua, don_gia_cong, chi_phi_khac_chuan))
                    st.success(f"✅ Đã thêm {ten_sp} thành công!")
                    time.sleep(1)
                    st.rerun()
                except Exception as e: st.error(f"⚠️ Mã hoặc Tên này đã tồn tại! Lỗi: {e}")
...
with tab2:
    ...
    try:
        # Lấy thêm cột chi_phi_khac để hiển thị và cho phép sửa
        df_sp = pd.read_sql("SELECT id, ma_sp, ten_sp, gia_dai_ly, gia_khach_le, dinh_muc_nhua, don_gia_nhua, don_gia_cong, chi_phi_khac, chi_tiet_bom FROM public.dm_san_pham ORDER BY id DESC", conn)
            ...
            if st.button("💾 Lưu Bảng Giá Chuẩn", type="primary"):
                for index, row in edited_sp.iterrows():
                    ...
                    c.execute("""UPDATE public.dm_san_pham 
                                 SET ma_sp=%s, gia_dai_ly=%s, gia_khach_le=%s, dinh_muc_nhua=%s, don_gia_nhua=%s, don_gia_cong=%s, chi_phi_khac=%s 
                                 WHERE id=%s""",
                              (str(row['ma_sp']), float(row['gia_dai_ly']), gia_kl, float(row['dinh_muc_nhua']), float(row['don_gia_nhua']), float(row['don_gia_cong']), float(row['chi_phi_khac']), int(row['id'])))
...
with tab4:
    ...
    try:
        df_ome = pd.read_sql("SELECT * FROM public.dm_san_pham_ome ORDER BY id DESC", conn)
        if not df_ome.empty:
...
with tab5:
    ...
    try:
        df_parts_chuan = pd.read_sql("SELECT ten_sp, dinh_muc_nhua, don_gia_nhua, don_gia_cong, chi_phi_khac FROM public.dm_san_pham WHERE chi_tiet_bom IS NULL", conn)
        df_parts_ome = pd.read_sql("SELECT ten_sp, dinh_muc_nhua, don_gia_nhua, don_gia_cong, chi_phi_khac FROM public.dm_san_pham_ome", conn)
    ...
    with st.form("form_add_bom", clear_on_submit=True):
        ...
        if st.form_submit_button("➕ Thêm vào Bộ"):
            if part_chon != "-- Chọn --":
                ...
                # Tính chi phí vốn lấy luôn cả chi phí khác đã nhập
                cpk = float(info_p['chi_phi_khac']) if pd.notna(info_p['chi_phi_khac']) else 0.0
                gia_von_1_cai = (float(info_p['dinh_muc_nhua']) / 1000 * float(info_p['don_gia_nhua'])) + float(info_p['don_gia_cong']) + cpk
    ...
    if st.session_state.bom_items:
        ...
        # ĐÃ CẬP NHẬT RÕ RÀNG Ô NÀY LÀ CHI PHÍ CHO CẢ BỘ SAU KHI LẮP RÁP
        cp_rap_bao_bi = col_cp1.number_input("Chi phí ráp bộ / Bao bì thùng (VNĐ/Bộ)", min_value=0.0, step=1000.0)
        bien_do_loi_nhuan = col_cp2.number_input("Biên độ lợi nhuận kỳ vọng (%)", value=20.0, step=5.0)
        ...
        if st.button("💾 CHỐT & LƯU SẢN PHẨM BỘ NÀY", type="primary", use_container_width=True):
            if not ma_bom.strip() or not ten_bom.strip():
                ...
                try:
                    ...
                    st.success(f"✅ Đã tạo thành công SP Bộ: {ten_bom}! SP này đã sẵn sàng ở trang Tạo Đơn Hàng.")
                    st.session_state.bom_items = []
                    time.sleep(2)
                    st.rerun()
