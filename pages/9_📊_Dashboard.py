...
st.header("📊 Báo Cáo Quản Trị & Hiệu Quả Kinh Doanh WANCHI")
...
try:
    ...
    # 2. Lấy từ điển Giá Vốn từ Kho Sản Phẩm (Chuẩn + OME)
    ...
    # Gộp 2 bảng lại thành 1 từ điển {Tên SP: Giá Vốn} để tra cứu siêu tốc
...
if not df_don_hang.empty:
    ...
    for _, row in df_don_hang.iterrows():
        ...
        try:
            if pd.notna(row['chi_tiet']) and str(row['chi_tiet']).strip() != "":
                ...
                for item in items:
                    # Nhận diện tên SP (hỗ trợ cả form Chuẩn và form OME)
                    ...
                    # Tra cứu Giá vốn từ từ điển, nếu không có thì mặc định bằng 0
    ...
    # Chuyển đổi định dạng Ngày để vẽ biểu đồ
    ...
    with col_chart2:
        ...
        # Lọc bỏ giá trị âm nếu lỡ bị lỗ để vẽ pie chart không lỗi
